...
from utils import IMG_SIZE, SPLIT_DIR, SEED

IMG_H, IMG_W = IMG_SIZE
AUTOTUNE = tf.data.AUTOTUNE

# Classification 4 classes : Painting et Photo sont fusionnés en "Other".
# Raison : la distinction Painting/Photo est déléguée au modèle binaire en aval ;
# la pénaliser ici biaiserait inutilement la sélection d'hyperparamètres.
CLASS_NAMES = ["Other", "Schematics", "Sketch", "Text"]   # Other = Painting ∪ Photo
N_CLASSES = len(CLASS_NAMES)

# Mapping dossier physique (data_split/<subset>/<folder>) -> index de classe groupée.
# Les dossiers Painting/ et Photo/ pointent tous deux vers "Other" (index 0).
_FOLDER_NAMES  = ["Painting", "Photo", "Schematics", "Sketch", "Text"]
_FOLDER_LABELS = [0, 0, 1, 2, 3]
...
def _get_label(file_path: tf.Tensor) -> tf.Tensor:
    ...
    n = tf.shape(parts)[0]
    folder = parts[n - 2]
    # Index du dossier physique parmi les 5 dossiers, puis remap vers la classe groupée.
    matches = tf.cast(tf.equal(folder, tf.constant(_FOLDER_NAMES)), tf.int32)
    folder_idx = tf.argmax(matches)
    grouped = tf.gather(tf.constant(_FOLDER_LABELS, dtype=tf.int32), folder_idx)
    return tf.cast(grouped, tf.int32)
