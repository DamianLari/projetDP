...
For each model:
  - model.summary()
  - Benchmark on test set (accuracy, loss, recall per class)

Usage:
    python3 analyze_tune_results.py

Generated plots:
  1. Bar chart: macro accuracy / loss / recall per model
  2. Scatter: accuracy vs number of parameters (performance/lightness ratio)
  3. Bar chart: hyperparameter importance (correlation with test_acc)
"""
...
# Common schema for benchmark_results.json (multiclass AND binary)
BENCHMARK_SCHEMA = [
...
SPLIT_DIR   = PROJECT_ROOT / "data_split"
...
def benchmark_model(model: tf.keras.Model, test_ds: tf.data.Dataset) -> dict:
    ...
    # One-vs-rest macro AUC (None if a class is missing from the test set)
    try:
...
# Hyperparameter importance (Pearson correlation with test_acc)
...
def plot_bar_metrics(df: pd.DataFrame, save_path: Path) -> None:
    """Bar chart: macro accuracy / loss / recall for each model."""
    n  = len(df)
    x  = np.arange(n)
    w  = 0.28
    ...
    ax.axhline(0.90, color="green", linestyle="--", linewidth=1.2, label="Target 90 %")
    ...
    ax.axhline(0.25, color="orange", linestyle="--", linewidth=1.2, label="Target 0.25")
...
def _scatter_acc_vs_params(ax, df: pd.DataFrame, param_col: str, xlabel: str) -> None:
    """Scatter subplot: accuracy vs a specific parameter column."""
    colors_ok = ["#2ecc71" if m else "#e74c3c" for m in df["meets_target"]]
    sc = ax.scatter(
    ...
    if not df_valid.empty:
        ...
        ax.scatter(best_row[param_col], best_row["test_acc"],
                   ...
                   label=f"Best ratio ({best_row['label']})", zorder=4)
...
def plot_accuracy_vs_params(df: pd.DataFrame, save_path: Path) -> None:
    """Two side-by-side scatter plots: deployed params vs training params."""
    ...
    _scatter_acc_vs_params(
        ...
        xlabel="Deployed params (trainable + non-trainable)"
    )
    axes[0].set_title("Deployment view", fontsize=11)

    _scatter_acc_vs_params(
        axes[1], df,
        param_col="n_params_train",
        xlabel="Training params (+ Adam optimizer state)"
    )
    axes[1].set_title("Training view (GPU RAM)", fontsize=11)
...
def plot_hp_importance(corr: pd.Series, save_path: Path) -> None:
    """Bar chart for hyperparameter importance (Pearson |correlation| with test_acc)."""
    ...
    ax.set_xlabel("|Pearson correlation| with test_accuracy", fontsize=10)
...
def plot_training_curves(history: dict, trial_label: str, save_path: Path,
    ...
    axes[0].axhline(0.25, color="orange", linestyle="--", alpha=0.6, label="Target 0.25")
    ...
    axes[1].axhline(0.90, color="green", linestyle="--", alpha=0.6, label="Target 0.90")
...
def plot_recall_per_class(df: pd.DataFrame, save_path: Path) -> None:
    """Heatmap: recall per class and per model."""
    ...
    fig.suptitle("Recall per class and per model", fontsize=13, fontweight="bold")
...
def _print_best(best_row: pd.Series, bench_results: list[dict], param_col: str) -> None:
    ...
    print(f"  Model            : {best_row['label']}")
    print(f"  Test acc         : {best_row['test_acc']:.4f}")
    print(f"  Test loss        : {best_row['test_loss']:.4f}")
    print(f"  Macro recall     : {best_row['recall_macro']:.4f}")
    print(f"  Macro F1         : {best_row['f1_macro']:.4f}")
    print(f"  Deployed params  : {int(best_row['n_params_deploy']):,}")
    print(f"  Training params  : {int(best_row['n_params_train']):,}")
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  {len(results)} models found in tune_results/")
    print(f"{'='*70}\n")
    ...
    for r in results:
        ...
        # Training parameters = + optimizer state (Adam moments m, v, step)
        n_training = sum(int(tf.size(v).numpy()) for v in model.variables)

        # summary
        print("  ▶ model.summary()")
        model.summary(line_length=80)
        print(f"  Deployed params   : {n_deployed:,}  (trainable + non-trainable)")
        print(f"  Training params   : {n_training:,}  (+ Adam optimizer state)")
        ...
        bench["n_params"]        = r["n_params"]      # from results.json (consistency)
        ...
        print("  Recall / Precision / F1 per class:")
    ...
    # Best accuracy / parameters ratio (both perspectives)
    df_valid = df[df["meets_target"]]
    if not df_valid.empty:
        # Deployed score
        ...
        print(f"\n{'='*70}")
        print("BEST ACCURACY / DEPLOYED PARAMS RATIO  (trainable + non-trainable)")
        print(f"{'='*70}")
        _print_best(best_dep, bench_results, "n_params_deploy")

        print(f"\n{'='*70}")
        print("BEST ACCURACY / TRAINING PARAMS RATIO  (+ Adam optimizer state)")
        print(f"{'='*70}")
        _print_best(best_tr, bench_results, "n_params_train")
    else:
        print("\n⚠ No model reached the target.")
    ...
    print(f"  Confusion matrices in: {FIGURES_DIR}/confusion_matrices/")
    print(f"  Training curves in: {FIGURES_DIR}/training_curves/")
...
def export_best_configs(bench_results: list[dict]) -> None:
    """Writes best_acc_config.json and best_efficiency_config.json (config_multiclass.json format),
    directly reusable without manual copy-pasting."""
    ...
    # Best performance/size ratio among those reaching the target (otherwise all).
    # Size = TOTAL number of params with Adam optimizer state (n_params_train).
    ...
    print(f"\n{'='*70}\nEXPORTED CONFIGS (ready for config_multiclass.json)\n{'='*70}")
    print(f"  Best accuracy: {best_acc['label']}  "
          f"acc={best_acc['test_acc']}  loss={best_acc['test_loss']}")
    print(f"    -> {acc_path}")
    print(f"  Best ratio       : {best_eff['label']}  "
